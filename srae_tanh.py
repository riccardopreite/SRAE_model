...
'''Sosteinable regressive Auto Encoder model and tanh activation'''
class SRAE(torch.nn.Module):
    ...
    def __init__(self, indicator_number=1, task_number=1, macro_number=1, batch_size=32, lr=3e-4, ae_loss="mse"):
        super().__init__()
        ...
        self.fc2_decoder = torch.nn.Linear(
            in_features=task_number,
        ...
        self.ae_optimizer = torch.optim.Adam(self.parameters(), lr=self.lr)
    ...
    '''Encoder with 3 to 1 layer'''
    def forward(self, ground_truth_input, ground_truth_index, train=False, validation=False, predict=False):
        '''Forward pass'''
        encoded = self.regressive(ground_truth_input)
        index_predicted = self.sigmoid(encoded)
        input_predicted = self.decoder(index_predicted)

    
        '''Computing loss if net is training'''
        ...
        return index_predicted.detach().clone().item(), input_predicted.detach().clone(), regressive_loss, ae_loss
    ...
    def dump_max_min(self, output_name):
        # json_string = json.dumps(self.max_min)
